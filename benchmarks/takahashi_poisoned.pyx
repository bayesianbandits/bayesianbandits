...
"""POISONED: introduces the ** minefield and missing cdivision to show impact.

Changes from baseline:
1. Uses ** (-2.0) instead of manual 1.0 / (x * x)  [Minefield 1]
2. Uses ** 0.5 instead of manual sqrt            (not applicable here, but
   the ** operator penalty still applies to any float exponent)
"""
import numpy as np
...
def takahashi_diagonal(
    ...
    for j in range(p):
        ...
        if col_len == 1:
            l_jj = data[indptr[j]]
            # MINEFIELD: ** (-2.0) instead of 1.0 / (l_jj * l_jj)
            z_diag[j] = l_jj ** (-2.0)
        else:
            n_sub = col_len - 1
            if n_sub > max_sub:
    ...
    for j in range(p - 1, -1, -1):
        ...
        l_jj = data[col_start]
        # MINEFIELD: ** (-1.0) instead of 1.0 / l_jj
        inv_l_jj = l_jj ** (-1.0)
        ...
        # MINEFIELD: ** 2.0 instead of inv_l_jj * inv_l_jj
        z_diag[j] = (inv_l_jj ** 2.0) * (1.0 + dot)

...
def _make_data(kind, n_features, n_obs=200, rng=None, density=None):
...
def _fit_estimator(estimator_type, kind, n_features, n_obs=200, density=None):
    ...
    if estimator_type == "glm_logit":
        ...
        if is_sparse:
            y = np.asarray(X_train @ true_coef).ravel()
        else:
            y = X_train @ true_coef
        y += 0.5 * rng.standard_normal(X_train.shape[0])
    else:
...
@pytest.fixture
def eb_normal_dense_100():
    return _fit_estimator("eb_normal", "dense", 100)


@pytest.fixture
def eb_normal_dense_100_fresh(eb_normal_dense_100):
    est, X_test, name = eb_normal_dense_100
    return lambda: (copy.deepcopy(est), X_test, name)


@pytest.fixture
def eb_normal_dense_1k():
    return _fit_estimator("eb_normal", "dense", 1_000)


@pytest.fixture
def eb_normal_dense_1k_fresh(eb_normal_dense_1k):
    est, X_test, name = eb_normal_dense_1k
    return lambda: (copy.deepcopy(est), X_test, name)


@pytest.fixture
def eb_normal_sparse_1k():
    return _fit_estimator("eb_normal", "sparse", 1_000)


@pytest.fixture
def eb_normal_sparse_1k_fresh(eb_normal_sparse_1k):
    est, X_test, name = eb_normal_sparse_1k
    return lambda: (copy.deepcopy(est), X_test, name)


@pytest.fixture
def eb_normal_sparse_100k():
    return _fit_estimator("eb_normal", "sparse", 100_000, density=0.001)

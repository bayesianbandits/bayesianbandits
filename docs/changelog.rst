...
- Policies declare what they *consume* rather than which sampling methods
  ...
  be set to combinations with no coherent meaning (#263)
...
- The ``elementwise`` opt-out on batch reward functions is removed, along
  ...
  nothing and does not widen (#263)
...
- ``InformationDirectedSampling``: a variance-based information-directed
  ...
  replacement, each slot re-solving the subgame of remaining arms (#262)
...
- ``sample_reward_space`` on ``NormalRegressor``,
  ...
  cost decision. Full-mode draws need no request: ``sample`` reduces
  through this route on its own whenever it is cheapest (see below)
  (#263)
